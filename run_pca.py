...
ax.yaxis.set_label_coords(-0.08,0.5)
plt.savefig("./results/correlation_1.png")


corr1 = np.zeros(df1.shape[1])
corr2 = np.zeros(df1.shape[1])
fig = plt.figure(figsize=(8,8))
ax = plt.subplot(111)
for i in range(1, df1.shape[1]+1):
	corr1[i-1] = np.correlate(transform[:, 0].tolist(), df1[str(i)].tolist())
	corr2[i-1] = np.correlate(transform[:, 1].tolist(), df1[str(i)].tolist())
ax.plot(corr1, 'o-', markersize=5)
ax.plot(corr2, 'o-', markersize=5)
plt.axvline(x=15, ymin=-30, ymax=90, color="black", linestyle=(0, (5, 10)))
plt.axvline(x=39, ymin=-30, ymax=90, color="black", linestyle=(0, (5, 10)))
plt.axvline(x=94, ymin=-30, ymax=90, color="black", linestyle=(0, (5, 10)))
plt.axvline(x=113, ymin=-30, ymax=90, color="black", linestyle=(0, (5, 10)))
plt.title('Correlation of PC1 and PC2 with each feature')
plt.xlabel('Feature')
plt.ylabel('correlation value')
ax.yaxis.set_label_coords(-0.08,0.5)
plt.savefig("./results/correlation_PC.png")